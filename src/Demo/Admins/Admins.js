...
class Admins extends React.Component {
    ...
    EditAdmin() {
        ...
        HtttpPutDefult("admins/" + selectedAdmin._id + "", selectedAdmin, true).then((res) => {
            if (res) {
    ...
    AddAdmin() {
        ...
        HtttpPostDefult("admins", newAdmin, true).then(async (res) => {
            if (!res.errors) {
                newAdmin._id = res.id
    ...
    delete(Item, key) {
        ...
        const { StoreAdmins } = this.props;
        HtttpDeleteDefult("admins/" + Item._id + "",null ,true).then((res) => {
            if (res) {
                Admins.splice(key, 1);
                this.setState({ Admins: Admins })
                StoreAdmins(Admins)
